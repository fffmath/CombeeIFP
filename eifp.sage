...
logging.basicConfig(filename='eifp.log', level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
def generate_eifp_instance(modulus_bit_length, alpha, gamma, delta, beta1, beta2, seed=None, max_attempts=10):
    """
    ...
    :param delta: The ratio of the bit length of a1 and a2
    ...
    while attempts < max_attempts:
        ...
        delta_bit_length = int(modulus_bit_length * delta)
        a1 = a2 = 2 ** int(modulus_bit_length * delta)
        share_bit = ZZ(randint(2**(share_bit_length - 1)+1, 2**share_bit_length - 1))
        logging.debug(f"The length of share_bit is {len(bin(share_bit)[2:])}")
        ...
        logging.debug(f"The length of p1 is {len(bin(p1)[2:])}")
        ...
        logging.debug(f"The length of p2 is {len(bin(p2)[2:])}")
        q1 = random_blum_prime(2**(q_bit_length - 1), 2**q_bit_length - 1)
        q2 = random_blum_prime(2**(q_bit_length - 1), 2**q_bit_length - 1)
        N1 = p1 * q1
        N2 = p2 * q2
        desired_solution = (int(bin(a1*p1)[2+int(modulus_bit_length*1-modulus_bit_length*alpha-modulus_bit_length*beta1):],2)* 2**int(beta2*modulus_bit_length-beta1*modulus_bit_length)-int(bin(a2*p2)[2+int(modulus_bit_length*1-modulus_bit_length*alpha-modulus_bit_length*beta2):],2),int(bin(MSB4p1)[2:],2)-int(bin(MSB4p2)[2:],2), q2, p2, a1)
        if N1.nbits() != modulus_bit_length or N2.nbits() != modulus_bit_length:
            attempts += 1
            seed = int(time.time() * 1e6)
            logging.info(f"Regenerated seed: {seed}")
        else:
            logging.info(f"Geenerated eifp instance successfully with seed: {seed}")
            logging.debug(f'share_bit: {bin(share_bit)[2:]}')
            logging.debug(f'p1: {p1}')
            logging.debug(f'MSB4a1p1: {bin(MSB4p1)[2:]}')
            logging.debug(f'share_bit4a1p1: {bin(a1*p1)[2+int(modulus_bit_length*1-modulus_bit_length*alpha-modulus_bit_length*gamma-modulus_bit_length*beta1):2+int(modulus_bit_length*1-modulus_bit_length*alpha-modulus_bit_length*beta1)]}')
            logging.debug(f'LSB4a1p1: {bin(a1*p1)[2+int(modulus_bit_length*1-modulus_bit_length*alpha-modulus_bit_length*beta1):]}')
            logging.debug(f'q1: {q1}')
            logging.debug(f'N1: {N1}')
            logging.debug(f'p2: {p2}')
            logging.debug(f'MSB4a2p2: {bin(MSB4p2)[2:]}')
            logging.debug(f'share_bit4a2p2: {bin(a2*p2)[2+int(modulus_bit_length*1-modulus_bit_length*alpha-modulus_bit_length*gamma-modulus_bit_length*beta2):2+int(modulus_bit_length*1-modulus_bit_length*alpha-modulus_bit_length*beta2)]}')
            logging.debug(f'LSB4a2p2: {bin(a2*p2)[2+int(modulus_bit_length*1-modulus_bit_length*alpha-modulus_bit_length*beta2):]}')
            logging.debug(f'q2: {q2}')
            logging.debug(f'N2: {N2}')
            logging.debug(f"The desired solution is (x0, y0, z0, w0, u0) = {desired_solution}")
...
def create_lattice(pr, shifts, bounds, order="invlex", sort_shifts_reverse=False, sort_monomials_reverse=False):
    ...
    """
    logging.info(f"Creating a lattice with {len(shifts)} shifts (order = {order}, sort_shifts_reverse = {sort_shifts_reverse}, sort_monomials_reverse = {sort_monomials_reverse})...")
    if pr.ngens() > 1:
...
def find_roots_groebner(N1, N2,pr,desired_solution, unknown_modular, polynomials,bounds):
    ...
    :param pr: the polynomial ring
    :param desired_solution: the desired root, just used for debug
    ...
    s = Sequence(polynomials[0:len(gens)], pr.change_ring(QQ,order='lex'))
    for ii in range(len(s)):
        logging.debug(f"Polynomials in Sequence: {s[ii]}")
        logging.debug(f'Test for Sequence: s(x0, y0, z0, w0, u0) mod modular = {s[ii](desired_solution)% unknown_modular}')
        logging.debug(f'Test for Sequence: s(x0, y0, z0, w0, u0) = {s[ii](desired_solution)} over Z')
...
def modular_eifp(desired_solution, unknown_modular, f, M, N1, N2, m, s, t, X, Y, Z, W, U):
    """
    Computes Generalized Extended Implicit Factorization Problem.
    ...
    for ii in range(m + 1):
        for jj in range(m - ii + 1):
            g = u ** (m - ii - jj) * (y*z) ** jj * w ** s * f ** (ii) * M ** (m - ii) * N1 ** max(t - ii, 0) * N2_inverse ** min(ii + jj, s)
            logging.debug(f'Initial polynomial shift: {g}')
            g = qr(g).lift()
            g = eliminate_N2(g, modular)
            shifts.append(g)
            logging.debug(f'Add shift: {g}')
            logging.debug(f'Test for shift: g(x0, y0, z0, w0, u0) mod modular = {g(desired_solution)% unknown_modular}')
    logging.info('Generating lattice for eifp')
    L, monomials = create_lattice(pr, shifts, [X, Y, Z, W, U])
    log_lattice(L)
    ...
    polynomials = reconstruct_polynomials(L, f, unknown_modular, monomials, [X, Y, Z, W, U])
    for poly in polynomials:
        logging.debug(f'Polynomials after reconstructing from short vectors: {poly}')
        logging.debug(f'Test for reconstruct_polynomial: g(x0, y0, z0, w0, u0) mod modular = {poly(desired_solution)% unknown_modular}')
        logging.debug(f'Test for reconstruct_polynomial: g(x0, y0, z0, w0, u0) = {poly(desired_solution)} over Z')
    #for roots in find_roots_variety(pr,desired_solution, unknown_modular, polynomials,[X, Y, Z, W, U]):
    #    yield roots[x], roots[y], roots[z], roots[w]
    for roots in find_roots_groebner(N1, N2,pr,desired_solution, unknown_modular, polynomials,[X, Y, Z, W, U]):
        yield roots[x], roots[y], roots[z], roots[w], roots[u]

def attack_eifp_instance(modulus_bit_length, alpha, gamma, delta, beta1, beta2, m=3,seed=None, s=None, t=None):
    """
    Attack an EIFP instance with parameters
    :param modulus_bit_length: the bit length of the modulus
    :param alpha, gamma, delta, beta1, beta2: parameters for the EIFP instance
    ...
    if beta2<beta1:
        ...
        beta1=beta1
        beta2=beta2
    N1_list, N2_list, share_bit, desired_solution = generate_eifp_instance(modulus_bit_length, alpha, gamma, delta, beta1, beta2, seed)
    N1 = N1_list[2]
    N2 = N2_list[2]
    x, y, z, w, u= ZZ["x", "y", "z", "w", "u"].gens()
    logging.info('Generating polynimial f')
    f = x*z + 2**(int(beta2*modulus_bit_length) + int(gamma*modulus_bit_length))*y*z + N2*u
    logging.debug(f'Polynomial f: {f}')
    X = Integer(2 ** int(beta2*modulus_bit_length))
    Y = Integer(2 ** int(modulus_bit_length*1+modulus_bit_length*delta-modulus_bit_length*alpha-modulus_bit_length*gamma-modulus_bit_length*beta1))
    ...
    U = Integer(2 ** int(delta*modulus_bit_length))
    logging.debug(f'M = {M}')
    ...
    for x0, y0, z0, w0, u0 in modular_eifp(desired_solution, unknown_modular, f, M, N1, N2, m, s, t, X, Y, Z, W, U):
        v = int(f(x0, y0, z0, w0, u0))
        if v == 0:
            p2 = w0
            q2 = z0
            a1 = a2 = u0
            p1 = (u0*w0+x0+y0*(2**int(gamma*modulus_bit_length+beta2*modulus_bit_length)))/(M * u0)
            q1 = N1/p1
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 8:
        print("Usage: sage eifp.sage <modulus_bit_length> <alpha> <gamma> <delta> <beta1> <beta2> <m>")
        sys.exit("Sorry, try again.")

    # Parse command-line arguments
    modulus_bit_length = int(sys.argv[1])
    alpha, gamma, delta, beta1, beta2 = map(RR, sys.argv[2:7])
    m = int(sys.argv[-1])
    result = attack_eifp_instance(modulus_bit_length, alpha, gamma, delta, beta1, beta2, m, seed)
    print(result)
